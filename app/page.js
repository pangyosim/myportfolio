...
const projects = [
  ...
  },
  {
    ...
    features:
      '핵심 기능은 Riverpod 기반 앱 상태 동기화, 정기결제 스케줄링/재처리(BullMQ + Redis), 구독 상태 변경 시 배치성 정합성 보정(Cron), 결제 작업의 중복 방지(jobId), Drizzle 트랜잭션 기반 결제 상태 업데이트, 그리고 AI 추천 시 후보군 검색 + RAG 기반 벡터 검증 후 모델 응답을 결합하는 파이프라인입니다.',
    retrospect:
