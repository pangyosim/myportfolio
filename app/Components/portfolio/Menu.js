
const Menu =[
  {
    id:1,
    image: "/img/oyesmainimg.png",
    title:"OTT 서비스 아이디 공유 프로젝트",
    category: "Project",
    content: "HTML5,CSS,Javascript으로 전반적인 웹을 구성하였으며,"+
            " Spring, Mybatis으로 로그인, 파티 등 CRUD를 구현하였습니다."+
            " 인기 TOP10구현에 JSOUP 데이터 크롤링을 활용하였습니다. "+
            " 파티 가입 시 PortOne 결제 API으로 테스트 결제를 구현하였습니다.",
    url: "https://github.com/pangyosim/strawberr7",
    iname: "fa-brands fa-github",
    buttontitle: "Github",
    site:"http://site.oyes.o-r.kr/"
  },
  {
    id:2,
    image: "/img/dwbbmainimg.png",
    title:"주변 은행 대기인원 현황 사이트",
    category: "Project",
    content:"공공데이터 IBK기업은행 대기현황 API, Naver maps API, 주차장 정보 API를 활용하여 현재 위치 근처 은행 대기현황, 주차장 현황을 Map에 표시해주는 웹사이트를 구현했습니다. 공지사항, Q&A, React localStorage 및 CRUD로 로그인 기능을 구현하였습니다.",
    url:"https://github.com/pangyosim/pizzapr0ject",
    iname: "fa-brands fa-github",
    buttontitle: "Github"
  },
  {
    id:3,
    image: "/img/velogimg.png",
    title:"마주했던 에러들을 정리한 velog",
    category: "Blog",
    content:"기록하지 않고 기억하는것은 언젠가 사라질 데이터가 될 위험이 있기 때문에 더욱 성장하기 위해 공부한 내용을 정리해서 올릴 블로그를 운영하기 시작했습니다.",
    url:"https://velog.io/@spg9468/posts",
    iname: "fa-brands fa-vimeo",
    buttontitle: "velog"
  },
  {
    id:4,
    image: "/img/notionpage.png",
    title:"공부했던 내용들을 정리한 Notion",
    category: "Blog",
    content:"교육 내용, 알고리즘, 개발공부들을 하며 결과물들을 노션에 저장하고 있습니다. 알고리즘을 공부할 때는 개념을 먼저 공부한 후 관련된 문제들을 푸는 것을 적어나가고 있습니다. 매일 학습 내용을 적는 습관을 들어 공부를 지속하고 있습니다.",
    url:"https://pgportfolio.notion.site/SIMPANGYO-DB-096dfd3881ea47bb8c48a4b4913f42af?pvs=4",
    iname: "fa-brands fa-neos",
    buttontitle: "notion"
  },
  {
    id:5,
    image: "/img/githubpage.png",
    title:"프로젝트들이 올라가 있는 Github",
    category: "Blog",
    content:"프로젝트, 개발공부들을 하며 결과물들을 깃허브에 저장하고 있습니다. 프로젝트를 진행할 때는 해당 프로젝트의 설계나 고민했던 것을 적어가고 있습니다.",
    url:"https://github.com/pangyosim",
    iname: "fa-brands fa-github",
    buttontitle: "Github"
  },
  // {
  //   id:6,
  //   Image: "https://i.ibb.co/zspXP6d/flank.png",
  //   title:"OYES 테스트 결제 API",
  //   category: "Sample"
  // },
  // {
  //   id:7,
  //   Image: "https://i.ibb.co/zspXP6d/flank.png",
  //   title:"FLANK a gaming app ",
  //   category: "Sample"
  // },
]
export default Menu